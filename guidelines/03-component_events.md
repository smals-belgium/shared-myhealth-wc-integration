...
**IMPORTANT:** a [data URL](https://developer.mozilla.org/en-US/docs/Web/URI/Reference/Schemes/data) is not a valid
base64-encoded `content` string. A host application may or may not handle it as expected, so don't count on it
and send only the pure base64 data without metadata.
...
By default the host app will determine the audience the same way as for any call to `getAccessToken()` and attach the
obtained token to the download request URL.

**IMPORTANT:** the `url` property of this API is not intended to work with 
[data URL](https://developer.mozilla.org/en-US/docs/Web/URI/Reference/Schemes/data)s. A host application may or may
not support them, so don't count on it. Use the `ClientDocument` payload instead.
...
| `share` | Share a document natively | `description`: Describe the document to the sharee. |
...
**SECURITY NOTICE:** Security concerns are the responsibility of the web component developers.  
It is their job to assess the risks of exposing documents through any of these events.  
The host application will just execute whatever's being requested by the web component.

Nevertheless, a host application may still decide to ignore requested actions for security reasons.  
For example, the MAGS mobile app will never allow sharing a document, as it would make it too trivially easy for a 
user to expose sensitive data to others.
