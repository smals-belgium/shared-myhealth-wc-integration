...
### Adjust custom events to built-in

Your component may currently be relying on custom events for actions that are now covered by the new built-in 
document events.

* Migrate those to the built-in events
* Don't forget to adjust the list of exposed `events` in the component manifest
* Notify host application owners that they can remove their current custom handlers
