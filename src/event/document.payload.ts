...
export type ClientDocument = Readonly<{
  ...
  /** The complete file name (including extension) to write to the device. */
 ...
 *
 * Document metadata like file name and mime type, are the reponsibility of the document service, and should be
 * sent to the client through standard response headers. A file name should be complete and contain an extension.
 *
 * The `url` can only be secured by the oidc authentication used by the host app.
 * In other words, the host app will determine the audience the same way as for any call to `getAccessToken()` and
 * attach the obtained token to the download request URL.
 *
